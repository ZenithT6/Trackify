...
class _SleepScreenState extends State<SleepScreen> with WidgetsBindingObserver {
  ...
  Future<void> _refreshSleepData() async {
    await HealthService.fetchAndSaveSleep();
    await HealthService.fetchAndSaveSleepStages();
    await _loadTodayStages();
  ...
  Future<void> _loadWeeklySleep() async {
    ...
    setState(() {
      ...
      todayHours = (history[todayKey] ?? sleepByWeekday[todayIndex] ?? 0.0).toDouble();
